...
from dataloader_retrival_text import S4Dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='/home/lihr/avs_2', type=str)
    ...
    logger.info("audio retrival threshold:{:.2f} \n" \
                "Dino text threshold:{:.2f} \n" \
                "Dino box threshold:{:.2f} \n".format(
        args.audio_threshold, args.dino_text_threshold, args.dino_box_threshold
                ))
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            ...
            gt_category = category_list[0]
            gt_category = gt_category.replace('_', ' ')
            gt_category = gt_category.replace('-', ' ')
            ...
            for i in range(len(image_paths)):
                ...
                scores = probs[:, 0]
                conf, indice = scores.topk(args.text_ret_num)

                text_prompt = categories[0]

                if args.text_ret_num > 1 :
                    for j in range(1, args.text_ret_num):
                        text_prompt = text_prompt + " and " + categories[j]
            ...
            save_mask(pred_masks, mask_save_path, category_list, video_name_list, vis_raw_img=True, raw_img_path=os.path.join(cfg.DATA.DIR_IMG, split))
