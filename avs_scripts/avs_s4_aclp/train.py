    ...
    for n, p in model.named_parameters():
        if not any([n.startswith(c) for c in conflicts]):
            if n.startswith('module.aclp'):
                backbone_model_params.append(p)
            else:
                head_model_params.append(p)
    # model_params = model.parameters()
    if(args.optimizer == 'Adam'):
        optimizer = torch.optim.Adam([{'params': head_model_params, 'lr': args.lr},
                            {'params': backbone_model_params, 'lr': args.lr * args.backbone_lr_mult}])
    elif(args.optimizer == 'AdamW'):
        optimizer = torch.optim.AdamW([{'params': head_model_params, 'lr': args.lr, 'weight_decay': args.weight_decay},
                            {'params': backbone_model_params, 'lr': args.lr * args.backbone_lr_mult, 'weight_decay': args.weight_decay}])
