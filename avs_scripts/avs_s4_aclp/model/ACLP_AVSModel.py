...
class Pred_endecoder(nn.Module):
    ...
    def forward(self, x, audio=None):
        ...
        conv5_feat = self.conv5(x0)    # BF x 256 x  7 x  7
        # print(conv1_feat.shape, conv2_feat.shape, conv3_feat.shape, conv4_feat.shape)

        feature_map_list = [conv1_feat, conv2_feat, conv3_feat, conv5_feat]
        a_fea_list = [None] * 4
        ...
        conv4_feat = self.path4(conv5_feat)            # BF x 256 x 14 x 14
        conv43 = self.path3(conv4_feat, conv3_feat)    # BF x 256 x 28 x 28
        conv432 = self.path2(conv43, conv2_feat)       # BF x 256 x 56 x 56
        conv4321 = self.path1(conv432, conv1_feat)     # BF x 256 x 112 x 112
