...
def save_mask(pred_masks, gt_masks, save_base_path, video_name_list, vis_raw_img=False, raw_img_path=None):
    ...
    for idx in range(bs):
        ...
        for video_id in range(len(one_video_masks)):
            ...
            output_gt_name = "%s.mp4_%d_gt.png"%(video_name, video_id+1)
...
def save_raw_img_mask(anno_file_path, raw_img_base_path, mask_base_path, split='test', r=0.5):
    ...
    for video_id in range(len(df_test)):
        ...
        raw_img_path = os.path.join(raw_img_base_path, category, video_name)
